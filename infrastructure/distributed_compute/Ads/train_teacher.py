...
def train_func(config):
    ...
    final_model = get_teacher()

    # Ray prepares models for DDP or FSDP
    # MUST UNDERSTAND BETTER ....
    # early_model = prepare_model(early_model)
    final_model = prepare_model(final_model)
    device = next(final_model.parameters()).device # model was loaded using ray, get which device was used to load the model
    
    # Get the specific shard for this worker
    train_data_shard = get_dataset_shard("train")

    # define optimizer for the model
    optimizer = get_optimizer(final_model, emb_lr=config["emb_lr"], dense_lr=config["dense_lr"])
    ...
    teacher_checkpoint = get_checkpoint()
    start_epoch = 0
    
    if teacher_checkpoint:
        with teacher_checkpoint.as_directory() as checkpoint_dir:
            ...
            model_to_load = final_model.module if hasattr(final_model, "module") else final_model
...
trainer = TorchTrainer(
    ...
    run_config=RunConfig(
        ...
        storage_path="s3://<BUCKET-NAME>/ads/finalRanker/",
